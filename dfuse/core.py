...
class Dfuse:
    ...
    def get_block_at_timestamp(
        self,
        time: datetime.datetime = datetime.datetime.now() - datetime.timedelta(1),
        comparator: str = "gte",
    ) -> BlockTimeStampType:
        """
        ...
        r = requests.get(
            f"{self.block_time_url}?time={time}&comparator={comparator}",
            headers=headers,
        )
    ...
    def fetch_abi(
        ...
        headers: dict = {"Authorization": f"Bearer {self.token}"}
        r = requests.get(
            f"{self.abi_url}?account={account}&json={json}&blok_num={block_num}",
            headers=headers,
        )
        if r.status_code == requests.codes.ok:
    ...
    def bin_to_json(
        self, account: str, table: str, hex_rows: list, block_num: int = None
    ) -> Bin2JSONType:
        """
    ...
    def get_permission_links(
        self, account: str, block_num: int = 0
    ) -> PermissionLinkType:
        """
        ...
        headers: dict = {"Authorization": f"Bearer {self.token}"}
        r = requests.get(
            f"{self.permission_links_url}?account={account}&block_num={block_num}",
            headers=headers,
        )
        if r.status_code == requests.codes.ok:
    ...
    def get_table(
        ...
        headers: dict = {"Authorization": f"Bearer {self.token}"}
        r = requests.get(
            f"{self.get_table_url}?account={account}&scope={scope}&table={table}&block_num={block_num}&key_type={key_type}&json={json}&with_abi={with_abi}&with_block_num={with_block_num}",
            headers=headers,
        )
        if r.status_code == requests.codes.ok:
    ...
    def get_table_row(
        ...
        headers: dict = {"Authorization": f"Bearer {self.token}"}
        r = requests.get(
            f"{self.get_table_row_url}?account={account}&scope={scope}&table={table}&primary_key={primary_key}&key_type={key_type}&block_num={block_num}&json={json}",
            headers=headers,
        )
        if r.status_code == requests.codes.ok:
    ...
    def get_table_scopes(self, account: str, table: str, block_num: int = 0):
        ...
        headers: dict = {"Authorization": f"Bearer {self.token}"}
        r = requests.get(
            f"{self.table_scopes_url}?account={account}&table={table}&block_num={block_num}",
            headers=headers,
        )
